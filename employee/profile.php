<?php
// filepath: c:\xampp\htdocs\parlor\employee\profile.php
...
function verify_password($user_id, $password, $conn) {
    // --- FIX: Initialize $hash to prevent "unassigned variable" warning ---
    $hash = null; 
    ...
    $stmt->fetch();
    $stmt->close();
    // If no user is found, $hash remains null, and password_verify correctly returns false.
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['photo_update'])) {
    if (isset($_FILES['profile_photo']) && $_FILES['profile_photo']['error'] == 0) {
        ...
        if (!in_array($file_type, $allowed_types)) {
            ...
            $target_dir = $_SERVER['DOCUMENT_ROOT'] . '/parlor/employee/assets/image/profile_photo/';
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['details_update'])) {
    ...
    if (!verify_password($employee_user_id, $current_password, $conn)) {
        ...
        try {
            $stmt1 = $conn->prepare("UPDATE users SET name = ?, phone = ? WHERE id = ?");
            $stmt1->bind_param("ssi", $name, $phone, $employee_user_id);
            $stmt1->execute();

            $stmt2 = $conn->prepare("UPDATE employees SET specialization = ? WHERE user_id = ?");
            $stmt2->bind_param("si", $specialization, $employee_user_id);
            $stmt2->execute();
            ...
            $conn->rollback();
            $details_err = "Error updating details: " . $exception->getMessage();
        }
    }
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['password_update'])) {
    ...
    if (!verify_password($employee_user_id, $current_password, $conn)) {
        ...
        $stmt->bind_param("si", $new_hash, $employee_user_id);
        $stmt->execute();
        $stmt->close();
...
// Fetch latest employee info for display
$stmt = $conn->prepare("SELECT u.name, u.email, u.phone, u.profile_photo, e.specialization FROM users u JOIN employees e ON u.id = e.user_id WHERE u.id = ?");
$stmt->bind_param("i", $employee_user_id);
$stmt->execute();
$emp = $stmt->get_result()->fetch_assoc();
$stmt->close();
$current_photo = $emp['profile_photo'] ? $emp['profile_photo'] : '/parlor/assets/images/default-avatar.png';
?>
<style>
            ...
            <div class="card shadow-sm">
                ...
                <div class="card-body">
                    ...
                    <form method="post">
                        ...
                        <div class="mb-3"><label for="name" class="form-label">Name</label><input id="name" name="name" class="form-control" value="<?= htmlspecialchars($emp['name']) ?>" required></div>
                        <div class="mb-3"><label for="email" class="form-label">Email (Cannot be changed)</label><input id="email" type="email" class="form-control" value="<?= htmlspecialchars($emp['email']) ?>" readonly></div>
                        <div class="mb-3"><label for="phone" class="form-label">Phone</label><input id="phone" name="phone" class="form-control" value="<?= htmlspecialchars($emp['phone']) ?>" required></div>
                        <div class="mb-3"><label for="specialization" class="form-label">Specialization</label><input id="specialization" name="specialization" class="form-control" value="<?= htmlspecialchars($emp['specialization']) ?>"></div>
                        <hr>
